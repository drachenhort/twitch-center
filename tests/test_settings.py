...
def test_live_notify_enabled_defaults_to_false():
    settings = Settings()
    assert settings.live_notify_enabled is False


def test_live_notify_enabled_reads_addon_setting():
    addon = xbmcaddon.Addon()
    addon.setSetting("live_notify_enabled", True)
    settings = Settings(addon=addon)
